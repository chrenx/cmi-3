...
class CMIDataset(Dataset):
    ...
    def __init__(self, opt, mode='train'):
        ...
        feats_filename = FEATS_1_filename if self.model_type == 1 else FEATS_2_filename

        self.data = {
                "gene_expr": np.load(os.path.join(opt.data_dir, feats_filename["gene_expr"])),
                "ab_titer": np.load(os.path.join(opt.data_dir, feats_filename["ab_titer"])),
                "cell_freq": np.load(os.path.join(opt.data_dir, feats_filename["cell_freq"])),
                "cytokine_olink": np.load(os.path.join(opt.data_dir, 
                                                       feats_filename["cytokine_olink"]))
            }
        
        if self.model_type == 2:
            self.data.update({
                "cytokine_legendplex": np.load(os.path.join(opt.data_dir, 
                                                            feats_filename["cytokine_legendplex"])),
                "tcell_activ": np.load(os.path.join(opt.data_dir, feats_filename["tcell_activ"])),
                "tcell_polar": np.load(os.path.join(opt.data_dir, feats_filename["tcell_polar"])),
            })
        print("good")
